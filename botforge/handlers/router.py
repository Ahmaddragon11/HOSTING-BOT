...
def build_handlers(app: Application, pm, scheduler, notifier):
    ...
    env_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(envmgr.start_add_env, pattern=r"^env_add:\w+$")
        ],
        states={
            ST_ENV_KEY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, envmgr.get_key)
            ],
            ST_ENV_VAL: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, envmgr.get_val)
            ],
        },
        fallbacks=[CommandHandler("cancel", mgr.cmd_cancel)],
        allow_reentry=True,
    ...
    sched_conv = ConversationHandler(
        ...
        states={
            ST_SCHEDULE_BOT: [
                CallbackQueryHandler(schedh.pick_bot, pattern=r"^spick:\w+$")
            ],
            ST_SCHEDULE_ACTION: [
                CallbackQueryHandler(schedh.pick_action, pattern=r"^sact:\w+$")
            ],
            ST_SCHEDULE_TIME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, schedh.get_time)
            ],
        },
        fallbacks=[CommandHandler("cancel", mgr.cmd_cancel)],
        allow_reentry=True,
    ...
    update_conv = ConversationHandler(
        ...
        states={
            ST_UPDATE_FILE: [
                MessageHandler(filters.Document.ALL, mgr.recv_update_file)
            ],
        },
        fallbacks=[CommandHandler("cancel", mgr.cmd_cancel)],
        allow_reentry=True,
    ...
    bc_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(bctrl.bc_start, pattern=r"^bc:[a-z_]+:\w+$")
        ],
        states={
            ST_BC_VALUE: [
    ...
    app.add_handler(
        CallbackQueryHandler(
            bctrl.on_cb, pattern=r"^(bot_ctrl|self_ctrl|bc_fetch|bc_del_photo):"
        )
    )

    # متغيرات البيئة
    app.add_handler(
        CallbackQueryHandler(envmgr.on_cb, pattern=r"^(env_menu|env_del|env_list):")
    )

    # الجدولة
    app.add_handler(
        CallbackQueryHandler(
            schedh.on_cb, pattern=r"^(sched_menu|sched_del|sched_list|sched_new):"
        )
    )
    ...
    app.add_handler(
        CallbackQueryHandler(
            searchh.on_cb, pattern=r"^(search_menu|filter_status:.+|search_result:.+)$"
        )
    )

    # الإشعارات
    app.add_handler(
        CallbackQueryHandler(notifh.on_cb, pattern=r"^(notif_menu|notif_toggle:.+)$")
    )

    # ── ملفات خارج المحادثة (drag & drop) ───────────────
    app.add_handler(
        MessageHandler(filters.Document.ALL & ~filters.COMMAND, mediah.on_doc_free)
    )
